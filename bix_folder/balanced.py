...
# Jump pose angles - adjust if robot falls or doesn't jump high enough
JUMP_CROUCH_THIGH = 1.4   # How much to bend thighs in crouch (higher = more crouch)
JUMP_CROUCH_CALF = -2.8   # How much to bend calves in crouch (more negative = more crouch)
JUMP_PUSH_THIGH = 0.2     # Thigh angle during push (lower = more extension)
JUMP_PUSH_CALF = -0.4     # Calf angle during push (closer to 0 = more extension)
JUMP_FLIGHT_THIGH = 0.5   # Thigh angle in flight (adjust for tucking)
JUMP_FLIGHT_CALF = -1.0   # Calf angle in flight (adjust for tucking)
JUMP_LAND_THIGH = 0.8     # Final thigh angle after landing
JUMP_LAND_CALF = -1.6     # Final calf angle after landing
...
def get_jump_pose(jump_elapsed):
    ...
    if jump_elapsed < JUMP_CROUCH_TIME:
        ...
        return {
            'hip': 0.0,
            'thigh': 0.8 + t * (JUMP_CROUCH_THIGH - 0.8),
            'calf': -1.6 + t * (JUMP_CROUCH_CALF - (-1.6))
        }
        ...
        return {
            'hip': 0.0,
            'thigh': JUMP_CROUCH_THIGH + t * (JUMP_PUSH_THIGH - JUMP_CROUCH_THIGH),
            'calf': JUMP_CROUCH_CALF + t * (JUMP_PUSH_CALF - JUMP_CROUCH_CALF)
        }
    elif jump_elapsed < JUMP_CROUCH_TIME + JUMP_PUSH_TIME + JUMP_FLIGHT_TIME:
        # Phase 3: Flight - legs tucked
        return {
            'hip': 0.0,
            'thigh': JUMP_FLIGHT_THIGH,
            'calf': JUMP_FLIGHT_CALF
        }
    else:
        ...
        return {
            'hip': 0.0,
            'thigh': JUMP_FLIGHT_THIGH + t * (JUMP_LAND_THIGH - JUMP_FLIGHT_THIGH),
            'calf': JUMP_FLIGHT_CALF + t * (JUMP_LAND_CALF - JUMP_FLIGHT_CALF)
        }
...
print("  - JUMP_*_THIGH/CALF: Adjust leg angles per phase")
