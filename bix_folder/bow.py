#!/usr/bin/env python3
"""
GO2-W STANDING UP / LAYING DOWN / BOW - MUJOCO NATIVE VIEWER
"""
...
def controller(model, data):
    ...
    # Build targets (with special handling for bow pose)
    TARGETS = {}
    for act_idx in range(12):
        joint_type = ACT_TO_JOINT[act_idx]
        
        # Check if this is a rear leg (RR or RL = actuators 6-11)
        is_rear_leg = act_idx >= 6
        
        # For bow pose, use different angles for rear legs
        if 'rear_thigh' in pose and is_rear_leg:
            if joint_type == 'thigh':
                TARGETS[act_idx] = pose['rear_thigh']
            elif joint_type == 'calf':
                TARGETS[act_idx] = pose['rear_calf']
            else:
...
def key_callback(keycode):
    ...
    if keycode in [85, 117]:  # U or u
        ...
        if current_pose != 'bow':
            target_pose = 'bow'
            transition_start_time = data.time
            print(f"Command: Bowing... (time={data.time:.2f})")
