...
def get_jump_pose(jump_elapsed, leg_actuator_idx):
    """
    Calculate pose for jumping motion
    leg_actuator_idx: 0-2 (FR), 3-5 (FL), 6-8 (RR), 9-11 (RL)
    """
    # Identify specific legs
    is_front_right = leg_actuator_idx in [0, 1, 2]   # FR
    is_front_left = leg_actuator_idx in [3, 4, 5]    # FL
    is_rear_right = leg_actuator_idx in [6, 7, 8]    # RR
    is_rear_left = leg_actuator_idx in [9, 10, 11]   # RL
    is_rear_leg = leg_actuator_idx >= 6
    
    if jump_elapsed < JUMP_CROUCH_TIME:
        # Phase 1: Crouch down - FRONT CROUCHES DEEPER
        t = jump_elapsed / JUMP_CROUCH_TIME
        
        if is_rear_leg:
            # Rear legs: shallower crouch + angle backward + slight outward
            hip_base = -0.2 * t
            if is_rear_right:
                hip_adjustment = 0.05
            elif is_rear_left:
            ...
            return {
                'hip': hip_base + hip_adjustment,
                'thigh': 0.8 + t * 0.4,
                'calf': -1.6 - t * 0.8
            }
        else:
            # Front legs: deeper crouch + slight inward to reduce power
            if is_front_right:
                hip_adjustment = -0.03  # Slight inward
            elif is_front_left:
                hip_adjustment = 0.03   # Slight inward (opposite direction)
            else:
                hip_adjustment = 0.0
        ...
        if is_rear_leg:
            # Rear: maintain hip adjustment for stable push
            if is_rear_right:
                hip_adjustment = 0.05
            elif is_rear_left:
                hip_adjustment = -0.05
            ...
            return {
                'hip': -0.2 + hip_adjustment,
                'thigh': 1.2 - t * 1.0,
                'calf': -2.4 + t * 2.0
            }
        else:
            # Front: maintain hip adjustment to balance forces
            if is_front_right:
                hip_adjustment = -0.03
            elif is_front_left:
                hip_adjustment = 0.03
...
def controller(model, data):
    ...
    if is_jumping:
        ...
        # Build targets for jump - MOVED INSIDE LOOP
        TARGETS = {}
        for act_idx in range(12):
            joint_type = ACT_TO_JOINT[act_idx]
            jump_pose = get_jump_pose(jump_elapsed, act_idx)  # Now act_idx exists!
    ...
    for act_idx in range(12):
        ...
        # Equal or slightly lower gains for front legs
        is_rear_leg = act_idx >= 6
        if is_rear_leg:
            torque = (KP * 1.5) * (target - pos) - KD * vel
        else:
            torque = (KP * 1.3) * (target - pos) - KD * vel  # Lower for front
